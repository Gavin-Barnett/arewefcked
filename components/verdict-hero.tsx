...
export function VerdictHero(props: {
  ...
  return (
    <section className="full-bleed relative py-3 sm:py-5" style={heroVars}>
      ...
      <div
        aria-hidden
        className="absolute inset-x-0 bottom-0 h-28 bg-gradient-to-b from-transparent to-[#070708]"
      />

      <div className="relative mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8">
        <div className="hidden justify-end pt-4 sm:flex">
          <p className="font-mono text-[0.68rem] text-ink/45 uppercase tracking-[0.28em]">
            {props.updatedLabel}
          </p>
        </div>

        <div className="grid gap-8 pt-3 pb-8 xl:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] xl:items-center">
          <div className="relative z-30 order-1 space-y-6 xl:pr-4">
            <div>
              <h1 className="hero-title-shadow font-semibold text-[clamp(3.5rem,8.5vw,6.3rem)] text-ink leading-[0.9] tracking-[-0.065em]">
                {props.heading}
              </h1>
              <p className="mt-5 max-w-2xl text-ink/78 text-lg leading-8 sm:text-xl">
                {props.description}
              </p>
            </div>
            <CountryAutocomplete
