...
import type { SourceAdapter, SourceFetchResult, SourceFetchScope } from "@/lib/sources/base";
...
type IndicatorKey = "inflation" | "unemployment" | "gdp_growth" | "fuel_imports";
...
const indicatorConfigs: IndicatorConfig[] = [
  ...
  },
  {
    ...
    describe: (value) => `fuel imports ${value.toFixed(1)}% of merchandise imports`,
...
async function fetchIndicator(scope: SourceFetchScope, config: IndicatorConfig): Promise<IndicatorSeries> {
  const countries = scope.mode === "global" ? starterCountries.map((country) => country.code) : [scope.country.code];
  const url = new URL(`https://api.worldbank.org/v2/country/${countries.join(";")}/indicator/${config.code}`);
  ...
  if (!response.ok) {
    throw new Error(`World Bank ${config.code} request failed with ${response.status}`);
  }

  const payload = await response.json();

  if (!Array.isArray(payload)) {
    throw new Error(`World Bank ${config.code} returned an unexpected payload shape.`);
  }
...
function normalizeCountryEvent(countryCode: string, series: IndicatorSeries[]) {
  ...
  const latestByIndicator = series
    .map((entry) => ({ entry, record: latestNonNullRecord(entry.records.filter((record) => record.country.id === countryCode)) }))
    .filter((entry): entry is { entry: IndicatorSeries; record: z.infer<typeof seriesRecordSchema> } => Boolean(entry.record));
  ...
  const totalSeverity = clamp(contributions.reduce((total, item) => total + item.score, 0), 0, 100);

  if (totalSeverity < 8) {
    return null;
  }

  const dominant = [...contributions].sort((left, right) => right.score - left.score)[0];
  const latestYear = contributions.map((item) => Number(item.year)).sort((left, right) => right - left)[0];
  const confidence = round(clamp(0.66 + latestByIndicator.length * 0.05, 0.66, 0.86), 2);
  ...
  return {
    ...
    metadata: {
      ...
      indicators: Object.fromEntries(
        contributions.map((item) => [item.key, { label: item.label, value: item.value, year: item.year, score: round(item.score, 1) }])
      )
    }
...
export const worldBankAdapter: SourceAdapter = {
  ...
  async fetch(scope): Promise<SourceFetchResult> {
    const settled = await Promise.allSettled(indicatorConfigs.map((config) => fetchIndicator(scope, config)));
    ...
    if (successes.length === 0) {
      ...
      return {
        ...
        health: {
          ...
          outageMessage: firstFailure instanceof Error ? firstFailure.message : "Unknown World Bank error",
          latencyMs: null,
          active: true,
          notes: "World Bank macro indicators failed during fetch or validation."
        }
      };
    }

    const countries = scope.mode === "global" ? starterCountries.map((country) => country.code) : [scope.country.code];
    const events = countries.map((countryCode) => normalizeCountryEvent(countryCode, successes)).flatMap((event) => (event ? [event] : []));
    const latestUpdated = successes
      .map((entry) => entry.lastUpdated)
      .filter((value): value is string => Boolean(value))
      .sort((left, right) => new Date(right).getTime() - new Date(left).getTime())[0] ?? null;
    const averageLatency = Math.round(successes.reduce((total, entry) => total + entry.latencyMs, 0) / Math.max(successes.length, 1));
    const degraded = failures.length > 0;
    const ageDays = latestUpdated ? (Date.now() - new Date(latestUpdated).getTime()) / (1000 * 60 * 60 * 24) : 999;
    ...
    return {
      ...
      notes: [
        ...
        ...(degraded ? [`${failures.length} indicator request${failures.length === 1 ? "" : "s"} failed during this refresh, so surviving macro baselines were kept.`] : [])
      ],
      health: {
        sourceKey: "world_bank",
        name: "World Bank macro indicators",
        status: degraded ? "degraded" : "operational",
        freshness: ageDays < 120 ? "fresh" : ageDays < 365 ? "delayed" : "stale",
        lastSuccessfulSync: toIsoString(latestUpdated),
        lastAttemptAt: new Date().toISOString(),
        outageMessage: degraded ? `${failures.length} World Bank indicator request${failures.length === 1 ? "" : "s"} failed during refresh.` : null,
