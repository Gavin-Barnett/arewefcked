...
const queryConfigs: QueryConfig[] = [
  {
    ...
    baseSeverity: 38,
    query: '(war OR missile OR drone OR shelling OR invasion OR airstrike OR frontline OR ceasefire)',
    keywords: ["war", "missile", "drone", "shelling", "invasion", "airstrike", "frontline", "ceasefire"]
  },
  {
    domain: "civil_unrest",
    label: "Civil unrest",
  ...
  },
  {
    ...
    label: "Governance",
    baseSeverity: 28,
    query: '(sanctions OR coup OR parliament OR emergency powers OR cabinet crisis OR martial law)',
    keywords: ["sanctions", "coup", "parliament", "emergency", "cabinet", "martial law"]
  },
  {
    domain: "macroeconomic",
    label: "Macroeconomic stress",
    baseSeverity: 26,
    query: '(inflation OR recession OR debt crisis OR currency shock OR cost of living OR fuel crisis OR energy prices OR food prices)',
    keywords: ["inflation", "recession", "debt", "currency", "cost of living", "fuel", "energy", "food"]
  },
  {
    domain: "cyber_infra",
    label: "Cyber & infrastructure",
    baseSeverity: 32,
    query: '(cyberattack OR ransomware OR blackout OR grid outage OR telecom outage OR internet shutdown)',
    keywords: ["cyberattack", "ransomware", "blackout", "grid", "telecom", "internet shutdown"]
  }
];
...
function scoreArticle(config: QueryConfig, title: string, occurredAt: string) {
  ...
  const keywordHits = config.keywords.filter((keyword) => normalizedTitle.includes(keyword)).length;
...
function normalizeArticle(scope: SourceFetchScope, config: QueryConfig, article: z.infer<typeof gdeltArticleSchema>) {
  const occurredAt = parseSeenDate(article.seendate);
  const countryCodes = resolveCountryCodesFromText(article.title, scope.mode === "country" ? scope.country.code : undefined);
  const severity = scoreArticle(config, article.title, occurredAt);
  const outlet = article.domain ?? "monitored outlet";
  const language = article.language ? article.language.toUpperCase() : "unknown language";
  ...
  return {
    ...
    confidence: 0.58,
    tags: [config.domain, ...(article.language ? [article.language] : []), ...(article.domain ? [article.domain] : [])],
    metadata: {
...
export function dedupeGdeltEvents(events: NormalizedEvent[]) {
  ...
  for (const event of events) {
    ...
    const keepExisting = (existing.severityNormalized ?? 0) >= (event.severityNormalized ?? 0);
    const dominant = keepExisting ? existing : event;
    const mergedCountries = [...new Set([...existing.countryCodes, ...event.countryCodes])];
...
export const gdeltAdapter: SourceAdapter = {
  ...
  async fetch(scope): Promise<SourceFetchResult> {
    const settled = await Promise.allSettled(queryConfigs.map((config) => fetchQuery(scope, config)));
    ...
    if (successes.length === 0) {
      ...
      return {
        ...
        health: {
          ...
          outageMessage: firstFailure instanceof Error ? firstFailure.message : "Unknown GDELT error",
          latencyMs: null,
          active: true,
          notes: "The GDELT current-events monitor failed during fetch or validation."
        }
      };
    }

    const events = dedupeGdeltEvents(successes.flatMap((result) => result.articles.map((article) => normalizeArticle(scope, result.config, article))));
    ...
    return {
      sourceKey: "gdelt",
      sourceName: "GDELT Global Event Monitor",
      sourceUrl: successes[0]?.url ?? "https://api.gdeltproject.org/api/v2/doc/doc",
      events,
      notes: [
        ...
        ...(degraded ? [`${failures.length} GDELT domain quer${failures.length === 1 ? "y" : "ies"} failed during this refresh, so surviving results were kept.`] : [])
      ],
      health: {
        sourceKey: "gdelt",
        name: "GDELT Global Event Monitor",
