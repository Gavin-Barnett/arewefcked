...
import {
  getMonitorPointsForCountry,
  globalMonitorPoints,
} from "@/lib/sources/monitor-points";
...
function buildClimateSeverity(
  weather: z.infer<typeof weatherResponseSchema>,
  air: z.infer<typeof airQualityResponseSchema>
) {
  const heatStress = clamp(
    ((weather.current.apparent_temperature - 30) / 18) * 55,
    0,
    55
  );
  const windStress = clamp(
    ((weather.current.wind_speed_10m - 40) / 60) * 20,
    0,
    20
  );
  const aqiStress = clamp((((air.current.us_aqi ?? 0) - 50) / 150) * 70, 0, 70);
  const particulateStress = clamp(
    (((air.current.pm2_5 ?? 0) - 12) / 120) * 25,
    0,
    25
  );

  return clamp(
    heatStress + Math.max(aqiStress, particulateStress) + windStress,
    0,
    100
  );
}
...
async function fetchPoint(country: CountrySummary) {
  ...
  if (!weatherResponse.ok) {
    throw new Error(
      `Open-Meteo weather request failed with ${weatherResponse.status}`
    );
  }

  if (!airResponse.ok) {
    throw new Error(
      `Open-Meteo air-quality request failed with ${airResponse.status}`
    );
  }
...
function normalizePointResult(
  input: Awaited<ReturnType<typeof fetchPoint>>
): NormalizedEvent {
...
export const openMeteoAdapter: SourceAdapter = {
  ...
  async fetch(scope): Promise<SourceFetchResult> {
    const points =
      scope.mode === "global"
        ? globalMonitorPoints
        : getMonitorPointsForCountry(scope.country.code);
    const settled = await Promise.allSettled(
      points.map((point) => fetchPoint(point))
    );
    ...
    return {
      ...
      sourceUrl:
        scope.mode === "global"
          ...
          : (successes[0]?.weatherUrl ?? "https://open-meteo.com/"),
      events,
      notes: [
        ...(scope.mode === "global"
          ? [
              "Global climate stress is sampled from a starter monitoring network rather than a full gridded planetary model in phase one.",
            ]
          : []),
        ...(degraded
          ? [
              `${failures.length} monitoring point${failures.length === 1 ? "" : "s"} failed during this refresh, so successful readings were preserved.`,
            ]
          : []),
      ],
      health: {
        ...
        freshness:
          Date.now() - latestTime < 1000 * 60 * 90 ? "live-ish" : "fresh",
        lastSuccessfulSync: toIsoString(new Date(latestTime)),
        lastAttemptAt: new Date().toISOString(),
        outageMessage: degraded
          ? `${failures.length} monitoring point${failures.length === 1 ? "" : "s"} failed during refresh.`
          : null,
