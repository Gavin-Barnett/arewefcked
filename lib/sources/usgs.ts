...
function buildSeverity(feature: z.infer<typeof usgsFeatureSchema>) {
  ...
  return clamp((magnitude - 4.5) * 18 + significance / 40 + Math.min(felt, 50) / 4 + tsunamiBonus + alertBonus, 0, 100);
}

function normalizeFeature(feature: z.infer<typeof usgsFeatureSchema>, countryCode?: string): NormalizedEvent {
  ...
  return {
    ...
    severityNormalized,
    confidence: 0.96,
    tags: ["earthquake", ...(feature.properties.tsunami ? ["tsunami"] : []), ...(feature.properties.alert ? [String(feature.properties.alert)] : [])],
    metadata: {
      longitude,
      latitude,
      depthKm,
...
async function fetchUsgsJson(scope: SourceFetchScope) {
  ...
  baseUrl.searchParams.set("starttime", new Date(Date.now() - 1000 * 60 * 60 * 24 * 7).toISOString());
  baseUrl.searchParams.set("endtime", new Date().toISOString());
  baseUrl.searchParams.set("minmagnitude", scope.mode === "country" ? "3.5" : "4.5");
...
export const usgsAdapter: SourceAdapter = {
  ...
  async fetch(scope): Promise<SourceFetchResult> {
    try {
      ...
      const events = json.features.map((feature) => normalizeFeature(feature, scope.mode === "country" ? scope.country.code : undefined));
      ...
      return {
        ...
        sourceUrl: url,
        events,
        notes: scope.mode === "country" ? ["Country seismic signals are approximated using a radius around the focal city until full boundary mapping is added."] : [],
        health: {
          sourceKey: "usgs",
          name: "USGS Earthquake Catalog",
          status: "operational",
          freshness: Date.now() - generatedAt.getTime() < 1000 * 60 * 30 ? "live-ish" : "fresh",
      ...
      return {
        ...
        health: {
          ...
          lastAttemptAt: new Date().toISOString(),
          outageMessage: error instanceof Error ? error.message : "Unknown USGS error",
          latencyMs: null,
          active: true,
